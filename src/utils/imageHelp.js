...
class imageHelp {
	handleImageUpdate(req, product) {
		...
		const { reqQueryForUpdate, reqQueryForAdd } = this.reqQueryTrim(
			req.query.image,
			product.arrayImage,
			1
		);
		const reqQueryForDelete = this.reqQueryTrim(
			req.query.imageDelete,
			product.arrayImage,
			0
		);
		...
		if (req.files.length != 0) {
			...
			if (reqQueryForUpdate.length != 0) {
				req.body.arrayImage = this.handleEditImage(
					product.arrayImage,
					reqQueryForUpdate.sort(),
					req.files
				);
				// add image
				if (reqQueryForAdd.length != 0) {
					req.body.arrayImage = this.handleNewImage(
						product.arrayImage,
						req.files,
						reqQueryForUpdate.sort(),
						"addAndUpdate"
		...
		if (reqQueryForDelete.length != 0) {
			req.body.arrayImage = this.handleDeleteImage(
				product.arrayImage,
				reqQueryForDelete.sort()
			);
		}
	...
	reqQueryTrim(reqQuery, arrImgShoe, flag) {
		...
		if (flag === 1) {
			var reqQueryForUpdate = reqQuery.filter(
				(position) => position < arrImgShoe.length
			);
			console.log(
				"🚀 ~ file: imageHelp.js ~ line 38 ~ imageHelp ~ reqQueryTrim ~ reqQueryForUpdate",
				reqQueryForUpdate
			);
			var reqQueryForAdd = reqQuery.filter(
				(position) => position >= arrImgShoe.length
			);
			console.log(
				"🚀 ~ file: imageHelp.js ~ line 40 ~ imageHelp ~ reqQueryTrim ~ reqQueryForAdd",
				reqQueryForAdd
			);
		...
		const reqQueryForDelete = reqQuery.filter(
			(position) => position < arrImgShoe.length
		);
	...
	handleEditImage(arrayImageOfShoe, arrayPositionOldImg, reqFiles) {
		const arrDeleteOldImg = arrayPositionOldImg.filter((positionOfImg) =>
			arrayPositionOldImg.includes(positionOfImg)
		);
		...
		arrDeleteOldImg.forEach((positionOfImg) => {
			fs.unlink(
				"src/public/upload/" + arrayImageOfShoe[positionOfImg].filename,
				(err) => console.log(err)
			);
			arrayImageOfShoe[positionOfImg].filename =
				reqFiles[positionUpdateImg].filename;
			positionUpdateImg++;
		});
		console.log(
			"🚀 ~ file: imageHelp.js ~ line 66 ~ imageHelp ~ handleEditImage ~ arrayImageOfShoe",
			arrayImageOfShoe
		);
	...
	handleNewImage(arrayImageOfShoe, reqFiles, positionOfNewImg, action) {
		...
		if (action === "addAndUpdate") {
			...
			arrayNewImage = this.addNewImgToArrImgOfShoe(
				arrayImageOfShoe,
				reqFiles,
				arrayImageOfShoe.length,
				positionOfNewImg.length
			);
		} else {
		...
		console.log(
			"🚀 ~ file: imageHelp.js ~ line 60 ~ imageHelp ~ handleNewImage ~ arrayImageOfShoe",
			arrayImageOfShoe
		);
		return arrayImageOfShoe;
	}

	addNewImgToArrImgOfShoe(
		arrayImageOfShoe,
		reqFiles,
		position,
		positionNewImage
	...
	handleDeleteImage(arrayImageOfShoe, arrayImageDelete) {
		...
		var arrImgToDelete = arrayImageDelete.filter((positionOfDelete) =>
			arrPositionOfImg.includes(parseInt(positionOfDelete))
		);
		// delete file name in storage (upload)
		arrayImageDelete.forEach((positionOfDelete) => {
			fs.unlink(
				"src/public/upload/" + arrayImageOfShoe[positionOfDelete].filename,
				(err) => console.log(err)
			);
		});
		// delete file name of image have request delete, not delete position
		arrImgToDelete.forEach((positionOfDelete) => {
			delete arrayImageOfShoe[positionOfDelete].filename;
		});
		// format arrayImg(position and filename orderly) when have delete
		arrayImageOfShoe = this.arrayImageFormat(arrayImageOfShoe);
		console.log(
			"🚀 ~ file: imageHelp.js ~ line 106 ~ imageHelp ~ handleDeleteImage ~ arrayImageOfShoe",
			arrayImageOfShoe
		);
		return arrayImageOfShoe;
	}
